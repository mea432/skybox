...
app.config['MAIL_PASSWORD'] = os.getenv('EMAIL')
app.config['MAIL_DEFAULT_SENDER'] = 'skyboxcloud0@gmail.com'
app.secret_key = "VERY-IMPORTANT-KEY"

mail = Mail(app)

conn = connect('/home/432mea/mysite/final.db',check_same_thread=False)
c = conn.cursor()
...
def sh():
  ...
  if request.method == 'GET':
    ...
    response = post(os.getenv('url')+"?wait=true", data={'thread_name': filename, "content": 'o', 'callback': 'skybox.mea-team.repl.co/callback'})
    try:
    ...
    if dbs(('SELECT Permission FROM Attachments WHERE ID = (?)',(f,)),True) != 2:
      try:
        if str(f) in uploadingfiles:
          ...
          lockdiscord.acquire(True)
          response = post(os.getenv('url') + '?thread_id=' + str(f), files={'file': request.data,})
...
def swh():
  if request.method == 'GET':
    ...
    response = post(os.getenv('url')+"?wait=true", data={'thread_name': request.headers.get('name'), "content": skein1024(init=l).hexdigest(), 'callback': 'skybox.mea-team.repl.co/callback'},files={"file":op})
    ID = response.json()["id"]
...
def encupload():
  ...
  if request.headers['last'] == 'true':
    ...
    try:
      ...
      return 'No file found with ID', 400
    k = request.data
    response = post(os.getenv('url') + '?thread_id=' + str(ID), files={'file': enc(encs[int(ID)],request.data)},data={"content":f"LAST:{ceil(len(k)/128)*128-len(k)}"})
    del encs[int(ID)]
    print(response.text)
    if response.ok:
      return 'success'
    ...
    response = post(os.getenv('url') + '?thread_id=' + str(ID), files={'file': enc(encs[int(ID)],request.data),})
    print(response.text)
    if response.ok:
      return 'success'
    else:
...
def retrievefile(id):
  ...
  if 0 == permission:
    ...
    if request.method == "GET":
      ...
      parts = loads(get(f"https://discord.com/api/v9/channels/{id}/messages/{id}",headers={"Authorization":os.getenv('Do')}).content.decode("utf-8"))
  ...
  def ret():
    if size > 10485760*99:
      ...
      for i in range(ceil(size/2500006400)):
        if first:
          parts = get(f"https://discord.com/api/v9/channels/{id}/messages?limit=100?",headers={"Authorization":os.getenv('Do')}).json()[::-1][1:]
          for i in range(99):
            if i == 98:
              before = parts[i]["id"]
          ...
          parts = get(f"https://discord.com/api/v9/channels/{id}/messages?limit=100&?after={before}",headers={"Authorization":os.getenv('Do')}).json()[::-1]
          if parts == []:
            return
          if len(parts) < 100:
            for i in parts:
      ...
      for i in get(f"https://discord.com/api/v9/channels/{id}/messages?limit=100",headers={"Authorization":os.getenv('Do')}).json()[::-1][1:]:
        yield get(i["attachments"][0]["url"]).content
  if 0 == permission:
    ...
    def enc_ret():
      ...
      if size > 10485760*99 + 128:
        ...
        for i in range(ceil(size/2500006400)):
          if first:
            for i in range(100):
              parts = get(f"https://discord.com/api/v9/channels/{id}/messages?limit=100",headers={"Authorization":os.getenv('Do')}).json()[::-1]
              if i == 99:
                before = parts[i]["id"]
            ...
            parts = get(f"https://discord.com/api/v9/channels/{id}/messages?limit=100&?before={before}",headers={"Authorization":os.getenv('Do')}).json()[::-1]
            if parts == []:
              return
            if len(parts) < 100:
              for i in range(len(parts)):
        ...
        f = get(f"https://discord.com/api/v9/channels/{id}/messages?limit=100",headers={"Authorization":os.getenv('Do')}).json()[::-1][1:]
        s = len(f)
        for i in range(s):
          if i == s-1:
  ...
  if current_user.is_authenticated:
    try:
      ...
      if owner[0] == current_user.username:
        f = delete(f'https://discord.com/api/v9/channels/{id}',headers={"Authorization": os.getenv('Do')})
